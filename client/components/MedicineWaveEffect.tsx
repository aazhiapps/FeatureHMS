...
export const MedicineWaveEffect = ({ 
  ...
  const medicineTypes = [
    { type: 'pill' as const, colors: ['#ff6b6b', '#4ecdc4', '#45b7d1', '#96ceb4', '#feca57'] },
    { type: 'capsule' as const, colors: ['#ff9ff3', '#54a0ff', '#5f27cd', '#00d2d3', '#ff9f43'] },
    { type: 'tablet' as const, colors: ['#ffffff', '#f1c40f', '#e74c3c', '#3498db', '#2ecc71'] },
    { type: 'liquid' as const, colors: ['rgba(52, 152, 219, 0.8)', 'rgba(155, 89, 182, 0.8)', 'rgba(46, 204, 113, 0.8)'] }
  ];

  const createTablet = (index: number): Tablet => {
    const typeData = medicineTypes[Math.floor(Math.random() * medicineTypes.length)];
  ...
  const createTabletElement = (tablet: Tablet): HTMLDivElement => {
    ...
    element.className = 'absolute pointer-events-none transition-all duration-100';
    ...
    switch (tablet.type) {
      case 'pill':
        ...
        element.style.boxShadow = '0 2px 4px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.3)';
        break;
      case 'capsule':
      ...
      case 'tablet':
        ...
        element.style.boxShadow = '0 2px 4px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.4)';
        // Add tablet line
